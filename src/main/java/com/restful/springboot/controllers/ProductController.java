...
public class ProductController {
    ...
    @GetMapping("/{id}/ano_compra")
    public ResponseEntity<Integer> getPurchaseYear(@PathVariable("id") UUID id) {
        Optional<ProductModel> productOptional = productService.findById(id);
        if (productOptional.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        Integer purchaseYear = productOptional.get().getano_compra();
        return ResponseEntity.ok(purchaseYear);
    }
}
